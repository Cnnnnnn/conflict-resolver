...
- **Undo 升级**：栈深度 5 → 20，每批采纳原子撤销（一次回退整批采纳的所有文件，而非单条），撤销提示展示 batch 标签（如 `pkg-lock.json × 4` / `5 个文件`）；
...
4. 使用命令面板执行：
   ...
   - `Conflict Resolver: Accept Selected (Current)` / `Accept Selected (Incoming)` / `Accept Selected (Both)`
   - `Conflict Resolver: Select All Conflicts` / `Clear Selection` / `Select Conflicts in File`
   - `Conflict Resolver: Show All / Source / Lock Conflicts`
   - `Conflict Resolver: Search Conflicts`（默认快捷键 `Ctrl+Shift+F`，聚焦面板时）
   - `Conflict Resolver: Next Conflict File` / `Previous Conflict File` / `Jump to First Conflict in Active File`
   - `Conflict Resolver: Stage All Resolved Conflicts`
   - `Conflict Resolver: Back to Previous Conflict`
   - `Conflict Resolver: Undo Last Accept`（默认快捷键 `Ctrl+Shift+Z`，撤销最近一批采纳）
   - `Conflict Resolver: Continue Merge/Rebase/Cherry-pick`（仅当处于合并/变基/拣选场景时显示）
...
- `ci.yml`：日常 PR / push main 时执行 `npm ci && npm run check`。
- `release.yml`：只在推送 `v*.*.*` tag 时触发，`npm ci && npm run check && vsce package --no-dependencies`，随后发布到 Marketplace / Open VSX 并创建 GitHub Release。
