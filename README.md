...
- **批量采纳**：勾选多个冲突后，顶部按钮可一键采用当前 / 传入；支持文件内全选、清空、Quick Pick 搜索定位；
...
4. 使用命令面板执行：
   ...
   - `Conflict Resolver: Accept Selected (Current)` / `Accept Selected (Incoming)`
...
- **采用当前** / **采用传入** 按钮：对当前选择执行批量处理（也支持在单个冲突上右键）。没有任何选择时，作用于所有可定位冲突。
- **文件右键**：可一键勾选该文件下的所有冲突。
